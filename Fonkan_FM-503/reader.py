...
class Reader():
    ...
    def write_user_memory(self, start_address: int, data_words: list) -> bool:
        ...
        for i, word in enumerate(data_words):
            word_hex = f"{word:04X}"  # converte cada word pra HEX
            cmd = f"\nW3,{start_address + i},{word_hex}\r".encode("utf-8")
            print(f"Enviando comando: {cmd.decode().strip()}")
            self.ser.write(cmd)

            # Aguarda e imprime todas as linhas de resposta
            while True:
                linha = self.ser.readline()
                if not linha:
                    # se chegou um tempo limite e não leu nada, encerra
                    break
                linha_decod = linha.decode().strip()
                if linha_decod:  # se não estiver vazia
                    print(f"Resposta: '{linha_decod}'")
                    # Se a linha for somente "X" ou não corresponder, valida aqui:
                    if linha_decod == "X":
                        print(f"❌ Falha ao gravar palavra {i}: {linha_decod}")
                        return False
                    # Se receber a confirmação (igual ao comando enviado sem os caracteres de controle)
                    if linha_decod == f"W3,{start_address + i},{word_hex}":
                        break
            # Após o while, se não houver confirmação, considera falha
        return True
